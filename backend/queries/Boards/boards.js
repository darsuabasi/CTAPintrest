...
const createBoard = async (req, res, next) => {
    const { board_name, board_description, creator_id, board_image } = req.body;  
    try {
        let newBoard = await db.one(`INSERT INTO Boards (board_name, board_description, creator_id, board_image) VALUES( $1, $2, $3, $4) RETURNING *`, [board_name, board_description, creator_id, board_image]);
        res.status(200).json({
            status: 'success',
            message: 'Yessir, your new board was created.',
            payload: newBoard
